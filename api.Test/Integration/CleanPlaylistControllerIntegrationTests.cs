...
public class CleanPlaylistControllerIntegrationTests : IntegrationTestBase
{
  ...
  public CleanPlaylistControllerIntegrationTests()
  {
    _cleanPlaylistServiceMock = new Mock<ICleanPlaylistService>();
    _loggerMock = new Mock<ILogger<CleanPlaylistController>>();

    _controller = new CleanPlaylistController(_cleanPlaylistServiceMock.Object, DbContext!, _loggerMock.Object);
  }

  private void SetupAuthenticatedUser(int userId, string supabaseId)
  {
    // For unit testing, we only need to mock the user authentication, not database operations
    var claims = new[]
    {
  ...
  public async Task GetUserJobs_WhenUserHasJobs_ShouldReturnJobs()
  {
    ...
    await DbContext!.CleanPlaylistJobs.AddRangeAsync(jobs);
  ...
  public async Task GetJobTrackMappings_WhenJobExists_ShouldReturnTrackMappings()
  {
    ...
    await DbContext!.CleanPlaylistJobs.AddAsync(job);
    await DbContext!.TrackMappings.AddRangeAsync(mappings);
    await DbContext!.SaveChangesAsync();
