...
public class PlaylistSyncService : IPlaylistSyncService
{
  ...
  public async Task<PlaylistSyncResult> SyncPlaylistAsync(PlaylistSyncConfig config)
  {
    ...
    try
    {
      ...
      _logger.LogInformation("Sync completed for config {ConfigId}. Added: {Added}, Removed: {Removed}, Time: {ElapsedMs}ms",
          config.Id, delta.TracksToAdd.Count, delta.TracksToRemove.Count, stopwatch.ElapsedMilliseconds);
  ...
  private async Task ApplyDeltaToPlaylistAsync(PlaylistSyncConfig config, PlaylistDelta delta, List<TrackMapping> newMappings)
  {
    ...
    if (tracksToAdd.Any())
    {
      _logger.LogInformation("Adding {TrackCount} tracks to playlist {PlaylistId}", tracksToAdd.Count, config.TargetPlaylistId);
    ...
    {
      _logger.LogInformation("Removing {TrackCount} tracks from playlist {PlaylistId}", delta.TracksToRemove.Count, config.TargetPlaylistId);
