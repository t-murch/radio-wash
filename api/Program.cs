...
if (!builder.Environment.IsEnvironment("Testing"))
{
  ...
  Console.WriteLine($"Configuring Hangfire with connection string: {connectionString?.Substring(0, Math.Min(50, connectionString?.Length ?? 50))}...");
  
  builder.Services.AddHangfire(config => config
      .SetDataCompatibilityLevel(CompatibilityLevel.Version_170)
      .UseSimpleAssemblyNameTypeSerializer()
      .UseRecommendedSerializerSettings()
      .UsePostgreSqlStorage(config => config.UseNpgsqlConnection(connectionString)));
  builder.Services.AddHangfireServer();
  
  Console.WriteLine("Hangfire server configured and added to services");
}
else
{
  Console.WriteLine("Skipping Hangfire configuration - Testing environment detected");
}
